...
local uuid_utils = require("penview.websocket.uuid")
	...
	_G["_WEBSOCKET_NVIM"].servers.callbacks[self.server_id] = {
		on_message = function(server_id, server_client_id, message)
			...
			if server.on_message then
				server.on_message(self, server_client_id, message)
			end
		end,
		on_client_disconnect = function(server_id, server_client_id)
			...
			if server.on_client_disconnect then
				server.on_client_disconnect(self, server_client_id)
			end
		end,
		on_client_connect = function(server_id, server_client_id)
			local server = WebsocketServerMap[server_id]
			if not server then
				error("Connected but client not found", server_id)
			end

			if server.on_client_connect then
				server.on_client_connect(self, server_client_id)
			end
		end,
		on_error = function(server_id, err)
			...
			if server.on_error then
				server.on_error(self, err)
			end
		end,
	}
	websocket_server_ffi.start(self.server_id, self.host, self.port, self.extra_response_headers)
end
